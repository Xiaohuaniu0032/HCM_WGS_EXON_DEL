...
#
# Design notes:
#   1. No temporary Step 1b / Step 1c shell scripts are generated.
#   2. CUSUM depth calling is delegated to:
#        bin/run_sample_cusum_depth.pl
#   3. Base-level depth-ratio calculation is delegated to:
#        bin/run_sample_base_depth_ratio.pl
...
my $candidate_bam_flank = get_conf_required(\%CONF, "CANDIDATE_BAM_FLANK");
validate_nonnegative_integer("CANDIDATE_BAM_FLANK", $candidate_bam_flank);

# CUSUM parameters are checked here for early failure.
# run_sample_cusum_depth.pl will also read them from config.
my %CUSUM = (
...
# HCM core gene list is required by several downstream scripts.
...
print_summary(
    ...
    candidate_bam_flank          => $candidate_bam_flank,
...
# ------------------------------------------------------------
# Generate one run.sh per sample
# ------------------------------------------------------------
...
sub write_sample_shell {
    ...
    my $candidate_bam_flank         = $args{candidate_bam_flank};
    ...
    # --------------------------------------------------------
    # Step 01. Gene mean depth and window-level depth candidates
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Step 03. Optional base-level depth ratio
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Step 04. Plot window-level depth ratio
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Step 05. Extract SA split reads
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Step 06. Cluster SA split reads
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Step 07. Optional TLEN extraction
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Step 10. Merge evidence
    # --------------------------------------------------------
    # merge_evidence.pl does not read config directly.
    # EVIDENCE_OVERLAP_FRACTION is read by HCMExonDel.pl
    # and passed here as --min-overlap.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Step 11. Extract candidate gene BAM
    # --------------------------------------------------------
    ...
    $cmd = join(
        ...
        "--flank",     shell_quote($candidate_bam_flank),
    );
    ...
    write_cmd(
        ...
        "Step 11: extract_candidate_gene_bam",
        $cmd,
        "$dir_ref->{log}/11.extract_candidate_gene_bam.log"
    );

    # --------------------------------------------------------
    # Step 12. Annotate candidates
    # --------------------------------------------------------
    ...
    write_cmd(
        ...
        "Step 12: annotate_candidates",
        $cmd,
        "$dir_ref->{log}/12.annotate_candidates.log"
    );
...
sub print_summary {
    ...
    print "CANDIDATE_BAM_FLANK         : $args{candidate_bam_flank}\n";
    print "\n";
...
Required:
  ...
  --bam-list
      TAB-delimited BAM list with two columns:
        SampleID    /absolute/path/to/sample.bam
...
Optional:
  ...
  --force
      Allow writing into existing sample output directories.
